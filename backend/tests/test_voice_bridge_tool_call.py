...
@pytest.mark.asyncio
async def test_handle_tool_call_uses_fallback_on_analysis_error(bridge):
    mock_live_session = AsyncMock()
    ...
    with patch("app.services.voice_bridge.fetch_all", AsyncMock(return_value=fake_data)), \
         patch("app.services.voice_bridge.compute_risk", return_value=fake_risk), \
         patch("app.services.voice_bridge.generate_voice_analysis", AsyncMock(side_effect=RuntimeError("API error"))):
